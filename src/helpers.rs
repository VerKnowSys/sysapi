...
pub fn print_header() {
    ...
    info!(
        "SysAPI {} - design, implementation: {} - licensed under: {}.",
        version.cyan(),
        CREATED_BY.cyan(),
        "MIT/BSD".cyan()
    );
    info!("");
    info!(
        "  This project is only a component of the '{}':",
        "ServeD-OS project".cyan()
    );
    info!(
        "  {}-driven, modern, production-quality operating-system, developed since 2011.",
        "HardenedBSD".cyan()
    );
    info!("");
    ...
    info!(
        "    - {} -> {}",
        "sysapi".cyan(),
        "https://github.com/VerKnowSys/sysapi".green()
    );
    info!(
        "    - {} -> {}",
        "svdOS".cyan(),
        "https://github.com/VerKnowSys/svdOS".green()
    );
    info!(
        "    - {} -> {}",
        "Sofin".cyan(),
        "https://github.com/VerKnowSys/sofin".green()
    );
    info!(
        "    - {} -> {}",
        "Sofin-definitions".cyan(),
        "https://github.com/VerKnowSys/sofin-definitions".green()
    );
    info!(
        "    - {} -> {}",
        "kvmpro".cyan(),
        "https://github.com/VerKnowSys/kvmpro".green()
    );
    info!(
        "    - {} -> {}",
        "Shable".cyan(),
        "https://github.com/VerKnowSys/Shable".green()
    );
    info!("");
    info!(
        "SysAPI: ControlPane URL: {}",
        format!("{}://{}", DEFAULT_CONTROLPANE_PROTOCOL, listen_address()).cyan()
    );
}
...
pub fn sanity_checks() {
    info!(
        "SysAPI: Validating availability of all mission-critical features of the underlying operating system…"
    );
    if !Path::new(DEFAULT_LIBKVMPRO_SHARED).exists() {
        warn!(
            "SysAPI: Shared library: '{}' is not installed yet!",
            DEFAULT_LIBKVMPRO_SHARED.red()
        );
        warn!(
            "SysAPI: This lib provides APIs of: '{}' and '{}' kernel features used to fetch processes data directly from the kernel.",
            "Kvm".red(),
            "procstat".red(),
        );
        warn!(
            "SysAPI: These APIs are: {} without that library!\n",
            "completely unusable and unavailable".red()
        );
        warn!(
            "HINT: To install library from provided git-module: {}, simply do:",
            "lib/kvmpro".cyan()
        );
        warn!(
            "      {}\n",
        ...
        warn!(
            "NOTE: You don't have to restart SysAPI server after installation of the shared-library."
        );
        warn!(
            "NOTE: Once shared-library will be installed - it will be loaded automatically.\n\n"
        );
    }
    ...
    if !Path::new(JAIL_BIN).exists() || !Path::new(JEXEC_BIN).exists() {
        error!(
            "SysAPI requires both 'jail' and 'jexec' utilities to be available in base system!"
        );
        panic!(
            "{}: Both 'jail' and 'jexec' system utilities are {}!",
            "FATAL ERROR".blue(),
            "mission-critical".red()
        );
    }
