...
impl Zone {
    ...
    pub fn validate_domain_addresses(
        from: &str,
        to: &str,
    ...
    /// Synchronously resolves first available IPv4 defined for given "domain":
    pub fn lookup_domain(domain: &str) -> Result<IpAddr, String> {
        ...
        match response {
            Ok(lookup) => {
                let addr = lookup.iter().next().unwrap_or(ip_localhost);
                info!(
            ...
            Err(err) => {
                warn!(
                    "Domain: {} resolve failed: {}. Fallback to localhost!",
                    domain.cyan(),
                    err.to_string().cyan()
                );
                Ok(ip_localhost)
